...
class UpdateHabitUseCase:
    ...
    async def execute(self, dto: UpdateHabitDTO) -> Habit:
        ...
        updated = Habit(
            ...
            if dto.habit_name is not None
            else habit.habit_name,
            habit_description=dto.habit_description
            if dto.habit_description is not None
            else habit.habit_description,
            frequency_id=dto.frequency_id
            if dto.frequency_id is not None
            else habit.frequency_id,
            category_id=dto.category_id
            if dto.category_id is not None
            else habit.category_id,
            habit_type_id=habit.habit_type_id,
            goal_target=dto.goal_target
            if dto.goal_target is not None
            else habit.goal_target,
            habit_start_date=habit.habit_start_date,
            habit_end_date=dto.habit_end_date
            if dto.habit_end_date is not None
            else habit.habit_end_date,
